...
// ============================================================================
  ...
  public static void main( String args[] ) {
   ...
   //ponemos otro bucle for para que el programa principal se espere que terminen todas las hebras
    ...
    tt = ( ( double ) ( t2 - t1 ) ) / 1.0e9;
    System.out.println("");
    System.out.println( "Programación paralelo circular-----------------------------------------"  );
    System.out.println( "Tiempo paralelo circular (seg.):                    " + tt );
    ...
    System.out.println( "Suma del vector X:          " + sumaX );
    System.out.println( "Suma del vector Y:          " + sumaY );
    System.out.println( "Fin del programa paralelo circular." );
  
  
    //------------------------------------------Implementación paralela con distribución por bloques--------------------------------------------
    
    inicializaVectorX( vectorX );
    inicializaVectorY( vectorY );
    t1 = System.nanoTime();
    
    //Creamos el vector de hebras
    MiHebraBloques[] vh2 = new MiHebraBloques[numHebras]; 
    
    //creamos las hebras y arrancamos
  	for (int i = 0; i<numHebras; i++){
  		vh2[i]=new MiHebraBloques(n,i,numHebras,vectorX, vectorY);
  		vh2[i].start();
  	}
  	...
  	t2 = System.nanoTime();
    double tDparalelaBloques = ( ( double ) ( t2 - t1 ) ) / 1.0e9;
    System.out.println("");
    System.out.println( "Programación paralelo bloques-----------------------------------------"  );
    System.out.println( "Tiempo paralelo por bloques (seg.):                    " + tDparalelaBloques );
...
class MiHebraBloques extends Thread{
	int n, miId, numHebras;
	double vectorX[];
	double vectorY[];
	MiHebraBloques(int n, int miId, int numHebras, double vectorX[],double vectorY[]){
		this.n = n;
		this.miId = miId;
		this.numHebras = numHebras;
		this.vectorX = vectorX;

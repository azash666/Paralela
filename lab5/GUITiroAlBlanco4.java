...
class MiHebraMultiDisparo extends Thread{

	JTextField jtxtMensaje;
	CanvasCampoTiro1a campoTiro;
	private String mensaje;
	private LinkedBlockingQueue<Proyectil4> cola;
	private List<Proyectil4> lista;
	private boolean salir;

	MiHebraMultiDisparo(JTextField jtxtMensaje, CanvasCampoTiro1a campo, LinkedBlockingQueue<Proyectil4> cola){

		this.jtxtMensaje = jtxtMensaje;
		this.campoTiro = campo;
		this.cola = cola;
		lista = new ArrayList<>();
		salir=false;
	}
	...
	public void run(){
		...
		while( !salir ){
			try{
				while (!cola.isEmpty() || lista.isEmpty()) lista.add(cola.take());
				...
				SwingUtilities.invokeAndWait( new Runnable(){
					...
					public void run(){
						...
						while(iter.hasNext()) {
							Proyectil4 proyectil = (Proyectil4) iter.next();
							proyectil.mueveDuranteUnIncremental( campoTiro.getObjetivoX(), campoTiro.getObjetivoY() );
							proyectil.dibujaProyectil( campoTiro );
							...
							if( proyectil.getEstadoProyectil() != 0 ) {
								...
								if( proyectil.getEstadoProyectil() == 2 ) {
									...
									mensaje = "Fallado. El objetivo esta en: " + campoTiro.getObjetivoX() + "  Has disparado a: " + proyectil.getIntPosX();
								}
...
public class GUITiroAlBlanco4 {
	...
	public void go() {
		SwingUtilities.invokeLater(new Runnable(){
			public void run(){
				LinkedBlockingQueue<Proyectil4> zonaDeIntercambio = new LinkedBlockingQueue<Proyectil4>();
				...
				Thread hebra = new MiHebraMultiDisparo(txfMensajes, cnvCampoTiro, zonaDeIntercambio);		//<------------------------------ Inicializacion del hilo
				hebra.start();
					...
					public void actionPerformed( ActionEvent e ) {
						...
						try {
							...
							if( ( 0.0 < angulo )&&( angulo < 90 )&&( velocidad > 0 ) ) {
								...
								// Dibuja el proyectil.
								Proyectil4 p = new Proyectil4( velocidad, angulo );
								zonaDeIntercambio.put(p);
/*					
								hebra = new MiHebraCalculadoraUnDisparo(p, txfMensajes, cnvCampoTiro, id);

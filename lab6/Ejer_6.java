...
class Ejer_6 {
	...
	public static void main( String args[] ) {
		...
		//
		// Implementacion con 6.
		//
		t1 = System.nanoTime();
		ConcurrentHashMap<String, Integer> chmCuentaPalabras = new ConcurrentHashMap<String,Integer>( 1000, 0.75F , 256);
	...
	// -------------------------------------------------------------------------
	public static void contabilizaPalabra( HashMap<String, Integer> chmCuentaPalabras, String palabra ) {
		chmCuentaPalabras.putIfAbsent(palabra, 0);
		AtomicInteger numVeces= new AtomicInteger(chmCuentaPalabras.get( palabra ));
		chmCuentaPalabras.replace( palabra, numVeces.get(), numVeces.incrementAndGet() );
...
class MiHebra_66 extends Thread{
	private int miId;
	...
	public void run(){
		for(int i = miId; i < vectorLineas.size(); i+=numHebras ) {
			...
			for( int j = 0; j < palabras.length; j++ ) {
				palabraActual = palabras[ j ].trim();
				if( palabraActual.length() > 0 ) {
					Ejer_5.contabilizaPalabra5(chmCuentaPalabras, palabraActual);
				}
			}
		}
	}

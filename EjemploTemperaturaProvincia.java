...
class EjemploTemperaturaProvincia {
	public static void main(String[] args) {
		...
		//
		secuencial(codProvincia, fecha);
		//
		// Implementación paralela Ejercicio 1
		//
		paralelaEjercicio1(codProvincia, fecha, numHebras);

	}

	private static void secuencial(int codProvincia, String fecha) {
		long t1;
		long t2;
		double ts;
		PuebloMaximaMinima MaxMin;
	...
	private static void paralelaEjercicio1(int codProvincia, String fecha, int numHilos) {
		long t1, t2;
		double ts;
		PuebloMaximaMinima MaxMin;
		System.out.println();
		...
		for(int i=0; i<numHilos; i++) {
			hilos[i] = new HiloEjercicio1(cola, fecha, MaxMin);
			hilos[i].start();
		}
		leeFicheroEj1(numHilos, cola);
		for(int i=0; i<numHilos; i++) {
			try {
				hilos[i].join();
		...
		t2 = System.nanoTime();
		ts = ( ( double ) ( t2 - t1 ) ) / 1.0e9;
		System.out.print( "Implementacion paralela Ejercicio 1.                           " );
		System.out.println( " Tiempo(s): " + ts );
		System.out.println( "  Pueblo: " + MaxMin.damePueblo() + " , Maxima = " +
				MaxMin.dameTemperaturaMaxima() + " , Minima = " +
				MaxMin.dameTemperaturaMinima() );
	...
	private static void leeFicheroEj1(int numHilos, BlockingQueue<Tarea> cola) {
		File           archivo = null;
		FileReader     fr      = null;
...
class Tarea {
	...
	public static Tarea envenenada() {
		Tarea devolver = new Tarea(-1);
		devolver.envenenada=true;
		return devolver;
	}
...
class HiloEjercicio1 extends Thread {
	BlockingQueue<Tarea> cola;
	String fecha;
	PuebloMaximaMinima maxMin;
	
	public HiloEjercicio1(BlockingQueue<Tarea> cola, String fecha, PuebloMaximaMinima maxMin) {
		this.cola=cola;
		this.fecha=fecha;
		this.maxMin = maxMin;
	}
	...
	public void run() {
		...
		while(!salir){
			...
			if(tarea.isEnvenenada()) salir=true;
			else
